...
if (isset($_POST['btn']) && $_POST['btn'] == 'cenovnik') {
    ...
    $service_desc = $mysqli->escape_string($_POST['service_desc']);

    $sql_cenovnik = "SELECT * FROM cenovnik WHERE salon_id='$salon_id' AND service_name='$service_name'";
    $result_cenovnik = $mysqli->query($sql_cenovnik);
    if ($result_cenovnik->num_rows > 0) {
        $_SESSION['fail_message'] = 'Već postoji usluga sa imenom <strong> ' . $service_name . '.</strong>';
        ...
        $sql_cenovnik_insert = 'INSERT INTO cenovnik (salon_id, service_name, service_dur, service_price, service_desc)'
            . "VALUES ('$salon_id', '$service_name', '$service_dur', '$service_price', '$service_desc')";
        ...
        $service_name_value = ' value=""';
        $service_price_value = ' value=""';
        $service_desc = '';
    }
} else {
    $service_name_value = ' value=""';
