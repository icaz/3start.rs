...
if (!isset($_GET['cen_id'])) {
    ...
    $cen_id = $_GET['cen_id'];
    $sql = "SELECT * FROM cenovnik WHERE id='$cen_id'";
    $result = $mysqli->query($sql);
    $service = $result->fetch_all(MYSQLI_ASSOC);
    $salon_id = $service[0]['salon_id'];
    $service_name = $service[0]['service_name'];
    $service_dur = $service[0]['service_dur'];
    $service_price = $service[0]['service_price'];
    $service_desc = $service[0]['service_desc'];
    $service_name_value = ' value="' . $service_name . '"';
    $service_price_value = ' value="' . $service_price . '"';
    $service_desc_value = ' value="' . $service_desc . '"';
...
if ($mysqli->query($sql)) {
    ...
    if ($result->num_rows > 0) {
        $saloni = $result->fetch_all(MYSQLI_ASSOC);
        $the_saloni[] = $saloni[0]['salon_id'];
        if (!in_array($salon_id, $the_saloni)) {
            //header('Location: provider_profile.php');
        }
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="card card-primary card-outline bg-dark">
                                ...
                                <div class="card-body border-top table-responsive">
                                    ...
                                    <form action="store_cenovnik.php" method="POST">
                                        <div class="form-row">
                                            ...
                                            <div class="col-md-3 mb-3">
                                                ...
                                                <select name="service_dur" id="service_dur" class="form-control custom-select" required>
                                                    <option value='1' <?php echo $min30; ?>>30 min</option>
                                                    <option value='2' <?php echo $min60; ?>>60 min</option>
                                                    <option value='3' <?php echo $min90; ?>>90 min</option>
                                                    <option value='4' <?php echo $min120; ?>>120 min</option>
                                                </select>
                                        ...
                                        <button type="submit" value="cenovnik" name="btn" class="btn btn-warning float-right"><i class="fas fa-plus"></i> izmeni<b>USLUGU</b></button>
                                    </form>
